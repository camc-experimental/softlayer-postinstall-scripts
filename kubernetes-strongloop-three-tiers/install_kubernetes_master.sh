  ...
  template:
    ...
    spec:
      ...
      - name: todolist-mongodb
        image: centos:latest
        command: ["/bin/bash","-c","cd; curl -kO https://raw.githubusercontent.com/camc-experimental/softlayer-postinstall-scripts/kubernetes-strongloop-three-tiers/kubernetes-strongloop-three-tiers/install_mongodb_in_centos_7.sh; bash install_mongodb_in_centos_7.sh $DBUserPwd"]
        ports:
        - containerPort: 27017
EOF
